...
namespace Codeworx.Identity.Test.Scopes
{
    public class AllowedScopeTest
    {
        ...
        public async Task TestAllowedScopsForOAuthAuthorizationRequest_AllowedMainScope_ExpectsOk()
        {
            ...
            var parameters = await ProcessRequestAsync(request, (r, u, i) => new AuthorizationParametersBuilder(r, u, i));
        ...
        public void TestAllowedScopsForOAuthAuthorizationRequest_ForbiddenMainScope_ExpectsError()
        {
            ...
            var exception = Assert.ThrowsAsync<ErrorResponseException<AuthorizationErrorResponse>>(async () => await ProcessRequestAsync(request, (r, u, i) => new AuthorizationParametersBuilder(r, u, i)));
        ...
        private async Task<TParameter> ProcessRequestAsync<TRequest, TParameter>(TRequest request, Func<TRequest, ClaimsIdentity, IUser, IIdentityDataParametersBuilder<TParameter>> builderfactory)
            where TParameter : IIdentityDataParameters
        {
            ...
            var idenittyUser = await sp.GetRequiredService<IUserService>().GetUserByIdentityAsync(user);

            var builder = builderfactory(request, user, idenittyUser);
