...
{
    public class AuthorizationCodeTokenRequestValidator : IRequestValidator<AuthorizationCodeTokenRequest>
    {
        public Task ValidateAsync(AuthorizationCodeTokenRequest request)
        {
            var error = false;

            if (!Validator.TryValidateProperty(request.Code, new ValidationContext(request) { MemberName = nameof(request.Code) }, new List<ValidationResult>()))
            {
                error = error || true;
            }
