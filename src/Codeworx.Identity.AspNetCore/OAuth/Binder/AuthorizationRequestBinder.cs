...
namespace Codeworx.Identity.AspNetCore.OAuth.Binder
{
    public class AuthorizationRequestBinder : AuthorizationRequestBinder<AuthorizationRequest>
    {
        ...
        protected override AuthorizationRequest GetResult(string clientId, string redirectUri, string responseType, string scope, string state, string nonce = null, string responseMode = null, string prompt = null)
        {
            return new AuthorizationRequest(clientId, redirectUri, responseType, scope, state, nonce, responseMode, prompt);
        }
    }
}